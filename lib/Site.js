...
module.exports = function (opts) {
    ...
    Site.prototype.getCategories = function () {
        ...
        this.posts.forEach(function (post) {
            (post.categories || []).forEach(function (category) {
                ...
                var cat = categories[category.name.toLowerCase()];
                if (cat === undefined) {
                    cat = category;
                    categories[cat.name.toLowerCase()] = cat;
                    cat.posts = cat.posts || [];
                    cat = category;
                    cats.push(cat);
                }
                ...
                cat.posts = _.sortBy(cat.posts, _.property("date")).reverse();
            });
        });
                ...
                if (Array.isArray(values)) {
                    ...
                    this._categories = values.map(function (item) {
                        return {
                            ...
                            file: slugFormat.replace("{category}", item).toLowerCase(),
                            name: item
                        };
                    }.bind(this));
                } else {
    ...
    var Paginator = function (site, posts, pageSize) {
        this.site = site;
    ...
    Object.defineProperty(Paginator.prototype, "posts", {
        get: function () {
            return this.site.filterPosts(this._posts).slice(0, this.pageSize);
        },
        enumerable: true,
        configurable: true
    });
    ...
    Site.prototype.homePaginator = new Paginator(site, site._posts);
